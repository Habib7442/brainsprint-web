...
interface GameState {
  ...
  startGame: (mode: string) => Promise<void>;
  ...
  startGame: async (mode) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('matches')
        .select('score, total_time, users(username, avatar_url)') // NOTE: users(name...) vs users(username...) depending on schema.
        // Assuming user metadata or users table structure. Mobile app used 'users(name, avatar_url)'.
        // My web schema uses 'full_name' or 'username'. I will select both or just stick to one.
        // I will trust the user that DB "fetch everything like mobile".
        // Mobile uses .select('score, total_time, users(name, avatar_url)')
        // I'll try that. If it fails, I might need to adjust.
        .eq('game_mode', mode)
        .gt('score', 0)
        .gt('total_time', 0)
      ...
      if (data && data.length > 0) {
        ...
        const name = (randomMatch.users as any)?.name || (randomMatch.users as any)?.username || 'Ghost';
