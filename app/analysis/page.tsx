...
export default function AnalysisPage() {
    ...
    const [historyFilter, setHistoryFilter] = useState<'All' | 'Maths' | 'Reasoning'>('All');
    ...
    const filteredSessions = useMemo(() => {
        return sessions.filter(s => {
            ...
            // Handle Maths mismatch (Maths vs Mathematics)
            if (filter === 'maths') {
                return category === 'maths' || category === 'mathematics';
            }
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Ensure sessions is always an array and filter out 'Formula Sprint' (Case Insensitive)
            const safeSessions = (sessionsData || []).filter(s => 
                s.sub_type?.toLowerCase() !== 'formula sprint' &&
                s.category?.toLowerCase() !== 'formula sprint'
            );

            setSessions(safeSessions); // Store filtered sessions
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2 text-sm text-gray-500 mt-1">
                                                ...
                                                {session.duration_minutes > 0 && (
                                                    <div className="flex items-center gap-1.5 text-orange-500 font-bold bg-orange-500/10 px-2.5 py-1 rounded-lg text-[10px] uppercase tracking-wider border border-orange-500/10 shrink-0">
                                    ...
                                    return (
                                        <div key={idx} className={`p-6 rounded-2xl border ${
                                            ...
                                            <div className="pl-12 space-y-3">
                                                ...
                                                <div className="mt-4 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 p-6 rounded-2xl border-2 border-blue-200/50 dark:border-blue-900/30">
                                                    <div className="flex items-center gap-2 mb-4 pb-3 border-b border-blue-300/30 dark:border-blue-800/30">
                                                        <Sparkles className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                                                        <span className="font-black text-blue-900 dark:text-blue-100 text-base uppercase tracking-wide">
                                                            Solution Steps
                                                        </span>
                                                    </div>
                                                    <ol className="space-y-4">
                                                        {q.explanation?.split('\n').filter((line: string) => line.trim()).map((step: string, idx: number) => {
                                                            // Remove existing numbering if present (e.g., "1. ", "2. ")
                                                            const cleanStep = step.replace(/^\d+\.\s*/, '');
                                                            return (
                                                                <li key={idx} className="flex gap-4 items-start">
                                                                    <span className="font-black text-xl text-blue-600 dark:text-blue-400 shrink-0 bg-blue-100 dark:bg-blue-900/30 w-8 h-8 rounded-full flex items-center justify-center">
                                                                        {idx + 1}
                                                                    </span>
                                                                    <div className="flex-1 pt-1">
                                                                        <div className="text-base leading-relaxed text-gray-800 dark:text-gray-200 font-medium">
                                                                            <MathRenderer text={cleanStep} />
                                                                        </div>
                                                                    </div>
                                                                </li>
                                                            );
